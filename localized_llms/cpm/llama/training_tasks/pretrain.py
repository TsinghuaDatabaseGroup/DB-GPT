...
from ..tokenizers import LlamaTokenizer
...
LlamaInputType = Union[str, Dict[str, "LlamaInputType"]]
...
_TransformFunction = Callable[[LlamaInputType, int, random.Random], LlamaInputType]


class LlamaBatch(TypedDict):
...
def convert_data_to_id(tokenizer, data: Any):
    input_ids = tokenizer.encode(data["input"])
    output_ids = tokenizer.encode(data["output"], add_special_tokens=False)
...
class _MixedDatasetBatchPacker:
    ...
    def apply_transform(
        self,
        data: LlamaInputType,
        transform: Union[Dict[str, Any], Callable[[LlamaInputType], LlamaInputType], None],
    ) -> LlamaInputType:
        ...
        def _expand_mapping(data: LlamaInputType, stars: List[str], path: List[str], target: List[str]):
            if len(path) == 0:
                num_stars = 0
                for it in target:
                    if it == "*":
    ...
    def _ensure_transform_function(self, module_name: str, transform_script_path: str) -> _TransformFunction:
        ...
        if transform_func is None:
            ...
            def _empty_transform_func(data: LlamaInputType, num_sample: int, r: random.Random):
    ...
    def build_instance(self, config: _MixedDatasetConfig):
        ...
        if isinstance(transforms, str):
            ...
            def _transform(data: LlamaInputType):
    ...
    def pack_batch(self, force: bool = False, unilm=False) -> LlamaBatch:
        # pack batch
    ...
    def add_data(self, config: _MixedDatasetConfig) -> Optional[LlamaBatch]:
        (
            input_ids,
            context,
            sample_ids,
...
class MixedDataset:
    ...
    def get(self) -> LlamaBatch:
        ret: LlamaBatch = self._q_data.get()  # type: ignore
