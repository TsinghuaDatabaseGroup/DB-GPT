...
def obtain_exceptions_in_times_with_metric_name(
    ...
    for metric_name, metric_values in detailed_metrics.items():
        ...
        # print("len(top5_abnormal_metrics): ", len(top5_abnormal_metrics))
        if is_abnormal:
            # maintain the top 5 abnormal metrics
            if len(top5_abnormal_metrics) < TOP_N_METRICS:
                # print(
                #     ">>> len(top5_abnormal_metrics): ",
                #     len(top5_abnormal_metrics),
                #     " TOP_N_METRICS ",
                #     TOP_N_METRICS,
                #     " len(top5_abnormal_metrics) < TOP_N_METRICS ",
                #     len(top5_abnormal_metrics) < TOP_N_METRICS,
                # )
                ...
                print(
                    ...
                    " len(top5_abnormal_metrics) < TOP_N_METRICS ",
                    len(top5_abnormal_metrics) < TOP_N_METRICS,
                    " top5_abnormal_metrics_map.values():\n ",
                    top5_abnormal_metrics_map.values(),
                    " \ntop5_abnormal_metrics_map\n ",
                    top5_abnormal_metrics_map,
                )
...
def fetch_prometheus_metrics(args):
    ...
    for alert in alerts:
        ...
        start_time = datetime.strptime(alert_time, "%Y-%m-%dT%H:%M:%S.Z").timestamp() - 60 * 5
