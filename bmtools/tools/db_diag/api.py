...
def get_index_result(algo, work_list, connector, columns,
    ...
    exp_conf_file = script_dir + \
        f"/optimization_tools/index_selection/selection_data/algo_conf/{algo}_config.json"
...
def build_db_diag_tool(config) -> Tool:
    ...
    def obtain_start_and_end_time_of_anomaly():
        ...
        try:
            ...
            ssh.connect(
                benchserver_conf.get("SERVER_ADDRESS"),
                username=benchserver_conf.get("USERNAME"),
                password=benchserver_conf.get("PASSWORD"))

            # Create an SFTP client
            sftp = ssh.open_sftp()

            # Change to the remote directory
            sftp.chdir(benchserver_conf.get("REMOTE_DIRECTORY"))
            ...
            for filename in files:
                remote_filepath = benchserver_conf.get("REMOTE_DIRECTORY") + \
                    '/' + filename
            ...
            file_content = sftp.open(
                benchserver_conf.get("REMOTE_DIRECTORY") +
                '/' +
                required_file_name).read()
    ...
    def cpu_diagnosis_agent(start_time: int, end_time: int):
        ...
        # + find_abnormal_metrics(start_time, end_time, monitoring_metrics, 'cpu')
...
""".format(
    ...
    def optimize_index_selection(start_time: int, end_time: int):
        ...
        schema_file = script_dir + \
            f"/optimization_tools/index_selection/selection_data/data_info/schema_job.json"
        workload_file = script_dir + \
            f"/optimization_tools/index_selection/selection_data/data_info/job_templates.sql"
