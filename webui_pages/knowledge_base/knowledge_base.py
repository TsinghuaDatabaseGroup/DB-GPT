...
def knowledge_base_page(api: ApiRequest, is_lite: bool = None):
    ...
    if selected_kb == "新建知识库":
        ...
        if st.session_state["selected_splits_info"]:
            st.write(f"<span style='color:#33333; font-size: 24px'>切片内容如下</span>", unsafe_allow_html=True)
            ...
            for item in response:
                with st.expander(item['file_name']):
                    for i, content in enumerate(item['contents']):
                        st.write(f"第{i+1}段：" + f"<div style='color:#666666'>{content}</div>", unsafe_allow_html=True)
