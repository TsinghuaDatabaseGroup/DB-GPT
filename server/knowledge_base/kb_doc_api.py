...
from server.knowledge_base.utils import (validate_kb_name, list_files_from_folder, get_file_path,
                                         files2docs_in_thread, KnowledgeFile, make_text_splitter)
...
def search_docs(
    ...
    data = [DocumentWithScore(page_content=x[0].page_content, metadata=x[0].dict()['metadata'], score=x[1]) for x in docs]
...
def docs_text_split_content(
    ...
    try:
        ...
        for file_name in file_names:
            ...
            results.append({"file_name": kb_file.filename, "contents": splited_doc_page_contents})
...
def fetch_expert_kb_names(
    ...
    kb_names = KBServiceFactory.get_services()
    if kb_names is None:
